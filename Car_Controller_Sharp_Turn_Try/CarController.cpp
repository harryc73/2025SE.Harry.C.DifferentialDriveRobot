...
CarController::CarController(LineSensor* lineSensor, Motor* motor, LedArray* VersionStatus)
{
  ...
  { 
    ...
    if((leftState < turnThreshold) && (rightState < turnThreshold)){
      motor->accelerate();
      turnStartTime = 0;
      VersionStatus->printGo();
    }
    else if((leftState > turnThreshold) && (rightState < turnThreshold)){
      if (turnStartTime == 0) {
